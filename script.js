...
function show(val) {
    ...
    if (val == 1) {
        box1.innerHTML = "Thank you for calling Aevon Tickets, this is Cluster speaking. How may I help you with your travel plans?";
    } else if (val == 2) {
        box1.innerHTML = "Hi, this is Cluster from Aevon Tickets. I'm calling regarding the flight inquiry you recently submitted. How are you today?";
    } else if (val == 3) {
        box1.innerHTML = "Hi, this is Cluster from Aevon Tickets promotional team. I'm calling because we have an ongoing promotion for your pending request.";
    } else if (val == 4) {
        box1.innerHTML = "Hi, this is Cluster from Aevon Tickets. I'm calling to see how things work with your plan to book a ticket. I've found some great new rates.";
    } else if (val == 5) {
...
function hardRestart() {
    ...
    const inputs = document.querySelectorAll('input, textarea, select, checkbox, .scriptbox1, .scriptbox2, .scriptbox3');
    inputs.forEach(input => {
  ...
  for (let i = 1; i <= 5; i++) {
        let box = document.getElementById("script-box" + (i === 1 ? "" : i)); 
        if (box) {
            box.innerHTML = "";
        }
    }
